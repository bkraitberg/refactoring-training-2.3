    ...
    public class UnitTests
    {
        ...
        public void Test_ErrorOccursWhenProductOutOfStock()
        {
            ...
            using (var writer = new StringWriter())
            {
                ...
                Assert.IsTrue(writer.ToString().Contains("has insuffcient quantity for that order"));
            }
        }

        [Test]
        public void Test_ErrorOccursWhenProductWhenOnlyOneInStock()
        {
            // Update data file
            List<Product> tempProducts = DeepCopy<List<Product>>(originalProducts);
        ...
        public void Test_ProductsWithZeroQuantityDoNotAppearInMenu()
        {
            // Update data file
            List<Product> tempProducts = DeepCopy<List<Product>>(originalProducts);
            tempProducts.Where(u => u.Name == "Nuts").Single().Qty = 0;
            ...
            using (var writer = new StringWriter())
            {
                ...
                Assert.IsFalse(writer.ToString().Contains(": Nuts"));
            }
        }
